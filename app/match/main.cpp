...
int main(int argc, const char *argv[]) {
    if (argc == 3) {
        ...
        vo_nono::Frontend::detect_and_compute(img1, kpt1, dscpts1, 500);
        vo_nono::Frontend::detect_and_compute(img2, kpt2, dscpts2, 500);
        std::vector<cv::DMatch> matches = vo_nono::Frontend::match_descriptor(
                dscpts1, dscpts2, 10, 40, 50);

        cv::Mat output_img1, output_img2;
        cv::drawMatches(img1, kpt1, img2, kpt2, matches, output_img1);
        matches = vo_nono::Frontend::filter_matches(matches, kpt1, kpt2, 3);
